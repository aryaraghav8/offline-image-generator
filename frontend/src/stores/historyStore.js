...
export const useHistoryStore = create((set, get) => ({
    ...
    toggleFavorite: (image) => set((state) => {
    ...
    filteredGenerations: () => {
        ...
        if (state.searchTerm) {
            ...
            filtered = filtered.filter((g) => g.images.some((img) => img.prompt.toLowerCase().includes(term)) ||
                g.id.toLowerCase().includes(term));
        }
        if (state.dateRange.from || state.dateRange.to) {
            filtered = filtered.filter((g) => {
                const genDate = new Date(g.createdAt);
                if (state.dateRange.from && genDate < state.dateRange.from)
                    return false;
                if (state.dateRange.to && genDate > state.dateRange.to)
                    return false;
                return true;
            });
        }
